...
// Standard: fetch ảnh qua URL (Zoho) với keep-alive, retry/backoff nhẹ,
// per-host concurrency = 10, transform ảnh bằng sharp (nếu có lợi).
...
const MAX_SOCKETS = Number(process.env.IMG_MAX_SOCKETS || 16);
...
/* ===== HTTP keep-alive ===== */
const httpAgent = new http.Agent({ keepAlive: true, maxSockets: MAX_SOCKETS, maxFreeSockets: MAX_SOCKETS });
const httpsAgent = new https.Agent({ keepAlive: true, maxSockets: MAX_SOCKETS, maxFreeSockets: MAX_SOCKETS });
...
function touchLRU(k: string) { const v = memCache.get(k)!; memCache.delete(k); memCache.set(k, v); }
...
async function writeDiskCache(url: string, buf: Buffer) { await ensureDir(DISK_CACHE_DIR); try { await fsp.writeFile(cachePath(url), buf); } catch {} }

/* ===== Transform ảnh bằng sharp (nếu có lợi) ===== */
async function transformImageIfNeeded(input: Buffer): Promise<Buffer> {
  ...
  if (meta.width > MAX_W || meta.height > MAX_H) pipe = pipe.resize({ width: MAX_W, height: MAX_H, fit: "inside", withoutEnlargement: true });

  let target = String(meta.format);
  if (CONVERT_PNG_PHOTOS && target === "png" && meta.hasAlpha !== true) target = PREFER_WEBP ? "webp" : "jpeg";
...
/* ===== Fetch + retry/backoff nhẹ ===== */
function sleep(ms: number) { return new Promise(r => setTimeout(r, ms)); }
function computeBackoff(attempt: number, retryAfter?: string | number) {
  if (retryAfter) { const s = Number(retryAfter); if (!Number.isNaN(s) && s > 0) return s * 1000; }
  const base = 400 * Math.pow(2, attempt - 1); // 400, 800, 1600...
  const jitter = Math.floor(Math.random() * 150);
  return Math.min(8000, base + jitter);
}
async function fetchRaw(url: string, token = "", attempts = 3): Promise<Buffer> {
  const headers = token ? { Authorization: `Zoho-oauthtoken ${token}` } : undefined;
  let lastErr: any;
  for (let i = 1; i <= attempts; i++) {
    try { const res = await httpClient.get(url, { headers }); return Buffer.from(res.data as ArrayBuffer); }
    catch (e: any) {
      lastErr = e;
      const s = e?.response?.status, code = e?.code;
      const retriable = s === 429 || (s >= 500 && s < 600) || code === "ECONNRESET" || code === "ETIMEDOUT" || code === "EAI_AGAIN";
      if (!retriable || i === attempts) break;
      const wait = computeBackoff(i, e?.response?.headers?.["retry-after"]);
      await sleep(wait);
    }
  }
  throw lastErr;
...
/* ===== Lấy ảnh (URL) + cache/coalesce ===== */
export async function getTransformed(url: string, token = ""): Promise<Buffer> {
  const now = Date.now();
  const ent = memCache.get(url);
  if (ent && ent.expiresAt > now) { if (ent.data) { touchLRU(url); return ent.data; } if (ent.promise) return ent.promise; }
  else if (ent) memCache.delete(url);

  const disk = await readDiskCache(url);
  if (disk) { memCache.set(url, { expiresAt: now + DISK_CACHE_TTL, data: disk }); touchLRU(url); evictIfNeeded(); return disk; }

  const p = (async () => { const raw = await fetchRaw(url, token); const t = await transformImageIfNeeded(raw); await writeDiskCache(url, t); return t; })();
  memCache.set(url, { expiresAt: now + DISK_CACHE_TTL, promise: p }); evictIfNeeded();

  try { const buf = await p; memCache.set(url, { expiresAt: Date.now() + DISK_CACHE_TTL, data: buf }); touchLRU(url); return buf; }
  catch (e) { memCache.delete(url); throw e; }
}

/* ===== Gom link ảnh từ JSON (chỉ http/https) ===== */
...
/* ===== Prefetch theo host-concurrency ===== */
async function prefetchTransformed(urls: string[], token = ""): Promise<void> {
  ...
  const tryStart = async (): Promise<void> => {
    ...
    for (let i = 0; i < queue.length; i++) {
      const h = hostOf(queue[i]); const used = byHostActive.get(h) || 0;
      if (used < HOST_CONCURRENCY) { pickedIdx = i; break; }
    }
    if (pickedIdx === -1) return;

    const url = queue.splice(pickedIdx, 1)[0]; const h = hostOf(url);
    globalActive++; byHostActive.set(h, (byHostActive.get(h) || 0) + 1);

    getTransformed(url, token).catch(() => {}).finally(() => {
      globalActive--; byHostActive.set(h, Math.max(0, (byHostActive.get(h) || 1) - 1)); void tryStart();
    });
  ...
  console.log(`[map-prefetch] urls=${urls.length} conc=${PREFETCH_CONCURRENCY} hostConc=${HOST_CONCURRENCY} took=${Date.now()-t0}ms`);
}
...
export function replaceHtmlTags(obj: any) {
  ...
  const walk = (o: any) => {
    if (Array.isArray(o)) for (let i = 0; i < o.length; i++) { const v = o[i]; if (typeof v === "string") { if (hasHtml(v)) o[i] = htmlToText(v); } else if (v && typeof v === "object") walk(v); }
    else if (o && typeof o === "object") for (const k of Object.keys(o)) { const v = (o as any)[k]; if (typeof v === "string") { if (hasHtml(v)) (o as any)[k] = htmlToText(v); } else if (v && typeof v === "object") walk(v); }
  };
  walk(obj);
  return obj;
}
...
export async function populateDataOnDocx({
  ...
  const opts: any = {
    ...
    getImage: async (tagObj: any) => {
      ...
      if (typeof url !== "string" || !/^https?:\/\//i.test(url)) throw new Error("Invalid image tag: link must be http(s) URL");
      return getTransformed(url, token);
    },
    getSize: (img: Buffer, tagObj: any) => {
      const meta = sizeOf(img);
      const width = Number(meta?.width || 1), height = Number(meta?.height || 1);
