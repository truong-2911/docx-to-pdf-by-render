...
// Reuse 1 profile cho tất cả request (nhanh hơn lần sau)
...
async function runSofficeMeasured(args: string[], opts: any = {}) {
  return new Promise<void>((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      clearInterval(tick);
      console.log(
        `[mem-peak] Node RSS: ${human(peakNode)} | LibreOffice: ${human(
          peakLO
        )} | TOTAL≈ ${human(peakNode + peakLO)}`
      );
      if (code === 0) resolve();
      else reject(new Error(`soffice exited ${code}. ${errBuf.trim()}`));
    });
  });
}
...
export async function convertDocxToPdf(
  ...
  try {
    if (!PERSISTENT_LO_PROFILE) {
      PERSISTENT_LO_PROFILE = await fsp.mkdtemp(
